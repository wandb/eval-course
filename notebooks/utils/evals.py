    ...
    for eval_call in eval_calls.children():
        if (
            ...
            scores = {
                "input": input_text,
                "required_keys": (
                    human_scores["human_required_keys"],
                    model_scores["test_adheres_to_required_keys"],
                ),
                "word_limit": (
                    human_scores["human_word_limit"],
                    model_scores["test_adheres_to_word_limit"],
                ),
                "privacy": (
                    human_scores["human_absence_of_PII"],
                    model_scores["judge_adheres_to_privacy_guidelines"],
                ),
                "overall": (
                    human_scores["human_overall_score"],
                    model_scores["judge_overall_score"],
                ),
            }
...
def calculate_kappa_scores(
    df,
    tuple_columns=["required_keys", "word_limit", "privacy", "overall"],
):
...
def calculate_weighted_alignment(kappa_scores, weights=None):
    ...
    weighted_score = sum(
        kappa_scores[metric] * weights[metric] for metric in kappa_scores.keys()
    )
